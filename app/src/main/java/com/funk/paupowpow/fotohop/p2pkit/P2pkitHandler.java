package com.funk.paupowpow.fotohop.p2pkit;

import android.app.Activity;
import android.util.Log;
import android.widget.Toast;

import com.funk.paupowpow.fotohop.config.FotohopConfigs;
...
public class P2pkitHandler  {
    ...
    private static final String APP_KEY = FotohopConfigs.APP_KEY;
    ...
    public void setP2pkitState() {
        if (FotohopDatabaseHandler.getInstance().isP2pkitStateEnabled()) {
            enableKit();
        } else {
            disableKit();
        }
...
//            Log.d(TAG, "with info: " + new String(peer.getDiscoveryInfo()));
    ...
    private void publishQuest() {
        FotohopDatabaseHandler dbh = FotohopDatabaseHandler.getInstance();
    ...
    private void receiveQuestInfoFromPeer(ch.uepaa.p2pkit.discovery.entity.Peer peer) {
        if (peer.getDiscoveryInfo() != null) {
            ...
            FotohopDatabaseHandler.getInstance().checkQuest(questInfo);
        }
    }
